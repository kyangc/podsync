...
describe("route auth boundaries", () => {
  ...
  it("serves the dashboard management shell with defensive headers", async () => {
    ...
    expect(body).not.toContain("id=\"refresh-dashboard\"");
    expect(body).not.toContain("sync-button");
    expect(body).not.toContain("sync-icon");
    ...
    expect(body).toContain(".summary { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr));");
