...
export function compileFeedsToml(feeds: FeedTomlRow[], youtubeDefaults: DownloaderDefaults): string {
  ...
  for (const feed of feeds) {
    ...
    if (feed.provider === "youtube") {
      chunks.push(
        `youtube_dl_args = ["--socket-timeout", "${youtubeDefaults.socket_timeout}", "--retries", "${youtubeDefaults.retries}", "--fragment-retries", "${youtubeDefaults.fragment_retries}", "--format", "18/best[protocol=https][vcodec!=none][acodec!=none]", "--extractor-args", "youtube:player_client=mweb", "--extractor-args", "youtubepot-bgutilhttp:base_url=http://bgutil-provider:4416"]`,
      );
    }
